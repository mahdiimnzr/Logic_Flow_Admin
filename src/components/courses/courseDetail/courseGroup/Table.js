...
import SidebarNewGroup from "./SideBar";
    ...
    <div className="invoice-list-table-header w-100 me-1 ms-50 mt-2 mb-75">
      <Row className="d-flex align-items-center justify-content-between">
        ...
        >
          ...
          <div className="d-flex align-items-center table-header-actions text-nowrap">
            <Button
...
const GroupsList = ({ data, isFetching }) => {
  const { t } = useTranslation();
  ...
  const displayData = useMemo(() => {
    ...
    return data.filter((value) =>
      value.groupName?.toLowerCase().includes(debounceSearch.toLowerCase())
    );
  }, [debounceSearch, data]);
  ...
  return (
    <Fragment>
      ...
      <SidebarNewGroup open={sidebarOpen} toggleSidebar={toggleSidebar} />
    </Fragment>
  );
};
