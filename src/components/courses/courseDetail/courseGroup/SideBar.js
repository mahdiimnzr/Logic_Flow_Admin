...
const SidebarNewGroup = ({ open, toggleSidebar }) => {
  ...
  const validationSchema = Yup.object({
    GroupName: Yup.string().required("FirstNameRequired"),
    GroupCapacity: Yup.string().required("LastNameRequired"),
  });
      ...
      <Form onSubmit={handleSubmit(onSubmit)}>
        <div className="mb-1">
          <Label className="form-label" for="GroupName">
            {t("GroupCapacity")} <span className="text-danger">*</span>
          </Label>
          <Controller
            name="GroupName"
            control={control}
