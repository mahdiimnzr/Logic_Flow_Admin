...
import SidebarNewUsers from "./LevelSideBar";
import { columns } from "./LevelColumns";
...
const CourseLevelList = ({ CourseLevel, isFetching }) => {
  const { t } = useTranslation();
  ...
  const displayData = useMemo(() => {
    ...
    if (!debounceSearch.trim()) return CourseLevel;
    return CourseLevel.filter((value) =>
      value.levelName?.toLowerCase().includes(debounceSearch.toLowerCase())
    );
  }, [CourseLevel, debounceSearch]);
