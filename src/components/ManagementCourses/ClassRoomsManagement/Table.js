...
const ClassRoomsList = ({ classRooms, isFetching }) => {
  const { t } = useTranslation();
  ...
  const displayData = useMemo(() => {
    ...
    if (!debounceSearch.trim()) return classRooms;

    return classRooms.filter((value) =>
      value.classRoomName.toLowerCase().includes(debounceSearch.toLowerCase()),
    );
  }, [classRooms, debounceSearch]);
