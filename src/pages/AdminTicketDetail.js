...
import { Card, CardHeader, CardBody, CardFooter, Input, Button, Spinner, Badge } from "reactstrap";
...
const AdminTicketDetail = () => {
    ...
    if (!ticketDetail) {
        return <div className="text-center py-5">تیکتی یافت نشد.</div>;
    }
            ...
            <Card className="chat-application">
                <CardHeader className="border-bottom d-flex justify-content-between align-items-center py-1">
                    <div className="d-flex align-items-center gap-1">
                        <Button color="flat-secondary" className="btn-icon rounded-circle" onClick={() => navigate(-1)}>
                        ...
                        <div>
                            ...
                            <small className="text-muted d-flex align-items-center gap-50 mt-25">
                                شناسه کاربر: {ticketDetail.userId} |
                                وضعیت: {isClosed ? (
                                    <Badge color="light-secondary" pill>بسته شده</Badge>
                                ) : (
                                    <Badge color="light-success" pill>باز</Badge>
                                )}
                            </small>
                        </div>
                    </div>
                ...
                <CardBody
                    className={`d-flex flex-column gap-2 overflow-y-auto ${skin === 'dark' ? 'bg-dark' : 'bg-light'}`}
                    style={{ height: "55vh" }}
                >
                        ...
                        <div style={{ maxWidth: "75%" }}>
                            <div className="d-flex align-items-center gap-50 mb-25">
                                <div className="avatar bg-light-secondary avatar-sm">
                                    <span className="avatar-content"><User size={14} /></span>
                                </div>
                                <small className="text-muted">کاربر</small>
                            </div>
                            <div
                                className={`p-1 rounded-3 shadow-sm ${skin === 'dark' ? 'bg-secondary text-white' : 'bg-white text-body'}`}
                                style={{ borderTopLeftRadius: "0" }}
                            >
                                <p className="mb-0 lh-base" style={{ whiteSpace: "pre-wrap" }}>{ticketDetail.describe}</p>
                            </div>
                            <small className="text-muted d-block mt-25" style={{ fontSize: "0.7rem" }}>
                                {ticketDetail.insetDate ? ticketDetail.insetDate.substring(0, 10) : ''}
                            </small>
                        </div>
                    </div>

                    {ticketDetail.ticketMessages && ticketDetail.ticketMessages.map((msg) => {
                        const isAdmin = msg.isSupport === true;
                        return (
                            <div key={msg.id} className={`d-flex ${isAdmin ? 'justify-content-end' : 'justify-content-start'}`}>
                                <div style={{ maxWidth: "75%" }}>

                                    <div className={`d-flex align-items-center gap-50 mb-25 ${isAdmin ? 'justify-content-end' : 'justify-content-start'}`}>
                                        {isAdmin ? (
                                            <Fragment>
                                                <small className="text-muted">شما (پشتیبان)</small>
                                                <div className="avatar bg-primary avatar-sm">
                                                    <span className="avatar-content"><MessageSquare size={14} /></span>
                                                </div>
                                            </Fragment>
                                        ) : (
                                            <Fragment>
                                    ...
                                    <div
                                        className={`p-1 rounded-3 shadow-sm ${isAdmin
                                            ? 'bg-primary text-white'
                                            : (skin === 'dark' ? 'bg-secondary text-white' : 'bg-white text-body')
                                            }`}
                                        style={isAdmin ? { borderTopRightRadius: "0" } : { borderTopLeftRadius: "0" }}
                                    >
                                        <p className="mb-0 lh-base" style={{ whiteSpace: "pre-wrap" }}>{msg.text || msg.message}</p>
                                    </div>

                                    <small className={`text-muted d-block mt-25 ${isAdmin ? 'text-end' : 'text-start'}`} style={{ fontSize: "0.7rem" }}>
                                        {msg.insertDate ? msg.insertDate.substring(0, 10) : ''}
                                    </small>
                    ...
                    {isClosed ? (
                        ...
                        <form onSubmit={handleSendMessage} className="d-flex align-items-center gap-1">
                            ...
                            >
                                ...
                                {isSending ? <Spinner size="sm" /> : <span className="d-none d-sm-inline">ارسال</span>}
                            </Button>
                        </form>
                    )}
                </CardFooter>
